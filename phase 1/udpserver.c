...
//		printf("%s\n", de->d_name); 
	...
	int len, recv_count;
	int termination;		// the result of processing buffer received
	...
	termination = procedure_on_receive(recv_count, recv_buffer, pszDirname, pszOutFilename, &seq_num, &ack_num);
	
	if (termination >= 0) // packet was received and processed
	{
		// build packet for acknowledgement
		int lengthof_packet = packet(2, ack_num, seq_num, (unsigned char *)send_buffer);
		printf("Client : %s\n", recv_buffer); 
		sendto(sockfd, (const char *)send_buffer, lengthof_packet, MSG_CONFIRM, (const struct sockaddr *) &clntaddr, len); 
